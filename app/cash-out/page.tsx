...
const consequences = [
  {
    label: "手数料で大幅に目減り",
    detail: "5万円分の商品を買っても、受け取れるのは3〜4万円程度。実質年利は数百%。",
  },
  {
    label: "支払い義務は残る",
    detail: "カードや後払いの請求はそのまま来る。翌月にはさらに苦しくなる。",
  },
  {
    label: "信用情報に傷がつく",
    detail: "支払い遅延でブラックリストに載り、ローンや賃貸契約に影響する。",
  },
  {
    label: "犯罪に加担するリスク",
    detail: "白ロム詐欺や口座売買に巻き込まれると、自分自身が犯罪者になる。",
  },
];
...
const alternatives = [
  ...
  {
    title: "フードバンク・こども食堂",
    description: "食料の支援を無料で受けられる。全国各地に窓口がある。",
    action: "「○○市 フードバンク」で検索",
  },
  {
    title: "法テラス（法律相談）",
...
export default function CashOutPage() {
  return (
    <>
      ...
      {/* ====== Warning banner ====== */}
      <div className="anim-up mb-10 overflow-hidden rounded-2xl bg-gradient-to-br from-red-500 to-rose-600 p-6 text-white">
      ...
      <section aria-labelledby="risks-heading" className="mb-12">
        ...
        <div className="stagger space-y-4">
          {risks.map((risk) => (
            <div key={risk.title} className="card overflow-hidden">
              <div className="p-4">
                <div className="mb-2 flex items-center gap-2">
                  <Ban className="h-4 w-4 shrink-0 text-danger" />
                  <h3 className="flex-1 text-base font-bold text-fg">
                    {risk.title}
                  </h3>
                  <span
                ...
                <div className="mt-3 h-1.5 overflow-hidden rounded-full bg-red-100">
                  <div
                    className="bar-fill h-full rounded-full bg-gradient-to-r from-amber-400 to-red-500"
                    style={{ "--bar-w": `${risk.pct}%` } as React.CSSProperties}
                  />
                </div>
              </div>
            </div>
      ...
      {/* ====== Consequences — numbered steps ====== */}
      <section aria-labelledby="consequences-heading" className="mb-12">
        <h2
        ...
        <div className="stagger space-y-3">
          {consequences.map((item, i) => (
            <div key={i} className="card flex items-start gap-3 p-4">
              <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-red-500 to-rose-500 text-xs font-bold text-white">
                {i + 1}
              </span>
              <div>
      ...
      {/* ====== Alternatives ====== */}
      <section aria-labelledby="alternatives-heading" className="mb-12">
        <h2
        ...
        <div className="stagger space-y-3">
          {alternatives.map((alt) => (
            <div
              key={alt.title}
              className="card overflow-hidden border-l-4 border-l-emerald-400 p-4"
            >
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <ul className="mt-3 space-y-2 text-sm">
              ...
              <li>
                多重債務相談窓口：{" "}
                <a href="tel:0570-031640" className="font-bold text-white underline">
                  0570-031640
                </a>
                （金融庁）
              </li>
